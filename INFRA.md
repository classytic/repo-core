...
| Arc 2.10 integration review | ☑ 2026-04-19 | — | Arc 2.10 accepts `MinimalRepo<TDoc> & Partial<StandardRepo<TDoc>>` structurally; CI gate at `arc/src/adapters/repo-core-compat.ts` + 14 HTTP round-trip tests across sqlitekit + mongokit. See `ARC_INTEGRATION_REPORT.md`. Three repo-core enhancements adopted: (1) `TExtra` generic on pagination result types, (2) `HOOK_EVENTS` constant, (3) `CacheAdapter.clear?(pattern?)` replacing `delByPattern?` (aligns with mongokit + arc). |
| M7 — arc refactor | ☑ 2026-04-19 | — | Done on arc side — `RepositoryLike` is now a proper superset of `MinimalRepo & Partial<StandardRepo>`. Dropped the `[key: string]: unknown` index sig + loosened `getDeleted` return type. Zero shims, zero kit-specific factories. |
